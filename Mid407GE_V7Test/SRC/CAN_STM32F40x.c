 ...
 *      Rev.:    V4.22  :modified by David, Jinfan 2012-1019
 *----------------------------------------------------------------------------
...
static U16 CAN1_filterIdx = 13;
static U16 CAN2_filterIdx = 13;
 ...
 *---------------------------------------------------------------------------*/
// CAN波特率   2012-10-20  DAVID
// tPCLK = 42MHz  
// CAN 波特率  = 外设时钟（tPCLK）/ (CAN分频值 * CAN位时间) 
//             = CAN时钟 / CAN位时间
// CAN 位时间  = _TSEG1+_TSEG2+1
#define     _SJW        (3)     // 同步跳变宽度
#define     _TSEG1      (4)     // 同步点到采样点延时时间
#define     _TSEG2      (2)     // 采样点到下个同步点的延时时间
...
static CAN_ERROR CAN_hw_set_baudrate (U32 ctrl, U32 baudrate)  
{
    ...
    /* Determine which nominal time to use for requested baudrate and set appropriate bit timing */
    if (baudrate <= 1000000)  {
//        brp  = (CAN_CLK / 7) / baudrate;
        brp  = (CAN_CLK) / (baudrate * (_TSEG1 + _TSEG2 + 1));
        ...
        CAN_set_timing(ctrl,  _TSEG1, _TSEG2, _SJW, brp);
    } else {
...
CAN_ERROR CAN_hw_setup (U32 ctrl)  
{
    ...
    #else
    //modified
        /* Enable clock for CAN1, GPIB */
        RCC->APB1ENR   |= RCC_APB1ENR_CAN1EN;
        RCC->AHB1ENR   |= RCC_AHB1ENR_GPIOBEN;
        /* CAN1, use PB8, PB9 */    //2012-1019
        // CAN接收脚PB8配置为上拉输入
        ...
        GPIOB->OSPEEDR |=   ( 2 << ( 8 << 1)) | ( 2 << (9 << 1));

        GPIOB->PUPDR   &= ~(( 3 << ( 8 << 1)) | ( 3 << (9 << 1)));
        GPIOB->PUPDR   |=   ( 1 << ( 8 << 1));  // PB8上拉
...
//        GPIOB->AFR[1]  |=   ( 9 << ( 5 << 2));
    ...
    case 2: 
        ...
        if (gSimCanBit & 0x01) {    // CAN1使用UART硬件，但必须开启CAN1 16-0520
            RCC->APB1ENR   |= RCC_APB1ENR_CAN1EN; 
        }
        /* Enable clock for CAN2 and GPIOB */    
        RCC->APB1ENR   |= RCC_APB1ENR_CAN2EN;
        RCC->AHB1ENR   |= RCC_AHB1ENR_GPIOBEN;
        /* CAN2, use PB5, PB6 */    //2012-1019
        // CAN接收脚PB5配置为上拉输入
        ...
        GPIOB->OSPEEDR |=   ( 2 << ( 5 << 1)) | ( 2 << (6 << 1));

        GPIOB->PUPDR   &= ~(( 3 << ( 5 << 1)) | ( 3 << (6 << 1)));
        GPIOB->PUPDR   |= ( 1 << ( 5 << 1));    // PB5上拉

        GPIOB->AFR[0] &= 0xf00fffff;
        GPIOB->AFR[0] |= 0x09900000;
...
#ifdef  EN_CANTX_INT
    CANx->IER   = CAN_IER_FMPIE0    /* FIFO 0 msg pending,  */
                | CAN_IER_TMEIE     /* Transmit mbx empty   */
                ;
#else
    CANx->IER   = CAN_IER_FMPIE0    /* FIFO 0 msg pending,  */
                ;
...
CAN_ERROR CAN_hw_tx_empty (U32 ctrl)    //2012-1107 modified
{
    CAN_TypeDef *CANx = CAN_CTRL[ctrl-1];


    if ((os_sem_wait (wr_sem[ctrl-1], 0) != OS_R_TMO)) 
    {   /* If semaphore is free*/
        ...
        else if ((CANx->TSR & (1 << 27)) != 0)  /* Transmit mailbox 1 is empty  */
        {
            EmptyBox = 1;
            return CAN_OK; 
        }
        else if ((CANx->TSR & (1 << 28)) != 0)  /* Transmit mailbox 2 is empty  */
        {
            EmptyBox = 2;
            return CAN_OK;
        }
...
CAN_ERROR CAN_hw_wr (U32 ctrl, CAN_msg *msg)  
{
    ...
    if (EmptyBox > 2) {
//        CANx->ESR |= 7 << 4;              // test
        CANx->TSR &= ~(TERR0|TERR1|TERR2);  // test
        return CAN_TX_BUSY_ERROR;
    }
    /* Reset TIR register   */
    CANx->sTxMailBox[EmptyBox].TIR  = (U32)0;   /* reset TXRQ bit */

    /* Setup the identifier information */
    if (msg->format == STANDARD_FORMAT)  {      /* Standard ID  */
        CANx->sTxMailBox[EmptyBox].TIR |= (U32)(msg->id << 21);
    } else {                                    /* Extended ID  */
        CANx->sTxMailBox[EmptyBox].TIR |= (U32)(msg->id <<  3) | (1 << 2);
    }

    /* Setup type information   */
    if (msg->type == REMOTE_FRAME)  {           /* REMOTE FRAME */
        CANx->sTxMailBox[EmptyBox].TIR |= (1 << 1);
    }

    /* Setup data bytes */
    CANx->sTxMailBox[EmptyBox].TDLR = (((U32)msg->data[3] << 24) 
                                    |  ((U32)msg->data[2] << 16) 
                                    |  ((U32)msg->data[1] <<  8)
                                    |  ((U32)msg->data[0])       
                                    ) ;
    CANx->sTxMailBox[EmptyBox].TDHR = (((U32)msg->data[7] << 24)
                                    |  ((U32)msg->data[6] << 16)
                                    |  ((U32)msg->data[5] <<  8)
                                    |  ((U32)msg->data[4])       
                                    ) ;
    /* Setup length */
    CANx->sTxMailBox[EmptyBox].TDTR &= ~0x0000000F;
    CANx->sTxMailBox[EmptyBox].TDTR |= (msg->len & 0x0000000F);
    ...
    /*  transmit message    */
    CANx->sTxMailBox[EmptyBox].TIR  |=  (1 << 0);   /* set TXRQ bit */
...
{
    CAN_TypeDef *CANx = CAN_CTRL[0];    //CAN_CTRL[ctrl-1];// 接收滤波由CAN1管理;
...
void CAN_hw_rx_object_add (U32 ctrl, U16 bank, U32 id, U32 object_para)  
{
    ...
    if ((CANx->FA1R & (1UL << bank)) == 0) {    /* filter bank unused ? */
        ...
        if ((object_para & FORMAT_TYPE) == STANDARD_TYPE) { /* Standard format used */ 
            ...
            CANx->FM1R |=  (0UL << bank);   //屏蔽位模式 /* Two 32-bit registers of filter bank x are in Identifier List mode */
            CANx->FS1R |=  (1UL << bank);   /* Single 32-bit scale configuration */

            CANx->sFilterRegister[bank].FR1 = (U32)(id << 3)                /* 32-bit identifier */
                                            | (1 << 2)                      /* IDE bit = 1 使用扩展标识符 */
                                            | (object_para & FRAME_TYPE);   /* RTR bit = 0 远程发送请求为0，即非远程帧 */

#ifdef  EN_CANRX_FILTER     // CAN接收滤波  14-0701
            if (Get_CANWorkInfo(ctrl-1) == MAINCHAN) {
                CANx->sFilterRegister[bank].FR2 = EXID_MASK << 3;
            } else {
                CANx->sFilterRegister[bank].FR2 = 0;    /* empty */
            }
#else
        ...
        &&      ((object_para & FORMAT_TYPE) == EXTENDED_TYPE)) {   /* Extended identifier must be added */
            /* check filter bank for free 32 bit filter */
                                                                    /* position n+0 is always used */
            if ((CANx->sFilterRegister[bank].FR2) == 0) {           /* check if position n+1 is used */
            CANx->sFilterRegister[bank].FR2 = (U32)(id << 3)        /* store 32-bit identifier on position n+1 */
                                            | (1 << 2) 
...
void CAN1_TX_IRQHandler (void) 
{
    ...
    if (CAN1->TSR & (1 << 0)) {     /* request completed mbx 0 */
        ...
        /* If there is a message in the mailbox ready for send, read the message from the mailbox and send it   */
...
void CAN1_RX0_IRQHandler (void) 
{
    ...
    if (CAN1->RF0R & 3) {   /* message pending ? */
        /* If the mailbox isn't full read the message from the hardware and send it to the message queue */
